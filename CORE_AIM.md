...
- **Cursor-anchored icon ring (added 2026-09-13, PROBLEM 267)**: With `middle_ring_style: icon_ring` (the default) a middle-button hold must raise a ring of the user's REAL app icons at the cursor — eight favourites at 45° on r=125 (`My eight`), or those eight plus every other bound letter and the specials on an outer ring (`All of them`, laid out by the tested `middle_ring::layout_ring`, never overlapping, never wider than 320 px) — clamped fully on-screen with the cursor warped to the centre, aim-to-highlight with the name in the centre pill, release (or left-click) to launch through the same cascade as Space+letter, release over nothing to close. Every tile shows a real icon: the shell's for apps and folders, the site's favicon for links (fetched ONCE when the link is bound, stored in the binding, never fetched at ring time; a letter disc only until then). `middle_ring_style: guide_hud` must restore phase 1 exactly. App exceptions carry a scope (Off entirely / Space only / Middle only); the built-in 3D/CAD rows default to Space only. UNPROVEN ON HARDWARE as of 2026-09-13.
