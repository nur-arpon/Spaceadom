...
## 2026-09-20 — Claude (Scrub-is-movement + toasts-never-stack 1.0.131 agent, Fable) — **1.0.131: scrub is MOVEMENT (one ←/→ tap per step of finger travel, in millimetres), and toasts never stack for the same thing (a fading live pill is revived, an identical ordinary toast restarts in place, the live linger is 1500 ms).** Owner decisions 2026-09-20 01:15; the 1500 ms linger via the coordinator. **SCRUB.** `touchpad/actions.rs`: the rate machinery is DELETED (`scrub_rate`, `scrub_cap`, `SCRUB_MIN_RATE`, `SCRUB_RAMP_FRACTION`, `scrub_tap`; `touchpad::scrub_pct` and the reader's `scrub_pending`/`dt`/`last_tick` with it). In its place ONE quantiser `quantise_steps(travel, dead_zone, step)` (chords' `chord_steps` is now a call to it) and ONE step engine `step_towards(&mut sent, target, send)` (the 1.0.122 up/down loops, shared by chords, presets and scrub; a still finger = same target = 0 sends; a lower target = backward sends). Scrub's step is in MILLIMETRES: `scrub_step_mm(s)` = `sens_scale(s, 12, 6, 2)` → 1 → 12, 2 → 10.5, 3 → 9, 4 → 7.5, **5 → 6**, 6 → 5.2, 7 → 4.4, 8 → 3.6, 9 → 2.8, 10 → 2 mm per tap; `scrub_step_size(s, short_mm)` divides by the pad's physical short side (`PadCaps::size_mm`, which the reader already had for the aspect; `None`/0/NaN → `SCRUB_FALLBACK_SHORT_MM` = 60, so the table reads 0.20 / 0.10 / 0.033 of the short side); `scrub_steps(travel_short_side, s, short_mm)` = `quantise_steps` with the 3 % dead zone (now 3 % of the SHORT side on every edge, the chords convention — decision where the ask was silent). Scrub IS `Chords { →, ← }` (`SCRUB_FORWARD`/`SCRUB_BACKWARD`); `Drive::Scrub | Drive::SeekFallback` run the same `scrub_steps` + `step_towards` arm; `chord_sent` is the hop count. Enter logs `touchpad: scrub — 6.0 mm per hop at sensitivity 5 (short side 74.0 mm, dead zone 3 % of it) (touchpad-scrub-movement-steps-spaceadom-131)`. Pill: "⏩ Scrub" → "⏩ +N" / "⏪ −N" (`scrub_hops_text`), no speed bar; the fallback keeps "⏩ Scrubbing (no seek bar here)" until its first hop, then counts the same way; `shows_steps(Scrub)` = true and the reader adds `drive.scrubs()` so a fallback gesture's `steps` reaches the page; `emit_live` takes `steps: Option<i32>`. Page `liveReadout`: scrub → hops where the percent goes ("+4"/"−2"), label "hops · Video scrub", no meter; fallback → hops, "hops · no seek bar here". **TOASTS.** New LEAF `src/components/toast-registry.ts` (imports nothing): `planLive(entries, key)` → create | update (dot/open) | revive (leave); `findIdentical(entries, text)` → the newest non-live open-or-fading entry with identical text. `toast.ts`: `ToastEntry` gains `text` and `key`; the `_live` map is gone (found by key over `_toasts`, so nothing to fall out of sync); `showLiveToast` = update (`setLiveText` only) / revive (`reviveEntry`: cancel timers, `leave` → `open`, same element and slot, then `setLiveText`) / create; `endLiveToast` no longer deletes the key (retire does) and arms `LIVE_LINGER_MS` = **1500** (+ OPEN_AT if still entering) then the normal exit; `showToast` checks `findIdentical` BEFORE `createElement` and, unless the pill is mid-flight or wearing SPACE, revives it, re-runs the drain ring (`restartDrain`), re-arms `duration` and beeps the open note (decision where silent) — different text still stacks. Rust: `LIVE_TOAST_KEY` = `"touchpad"` (was `"touchpad-slide"`), one key for all edges; `"app-volume"` unchanged. **Tests:** `cargo test --release --lib` — the mm table, `scrub_step_size` with/without a size, light travel → 0, leaving the dead zone → exactly 1 for the whole first step, +1 step → 2, back past the landing → negative, `step_towards` × 50 same target → 0 sends, 0 → 3 back / −2 → 2 more, an end-to-end travel sequence, a bad step → 0, the hop pills, `shows_steps`; `scripts/toast-registry.test.ts` (new, 6); `scripts/live-toast.test.ts` re-pinned (8: plan-before-create, update touches text only, revive cancels + un-fades, 1500 ms, identical-text restart, one key). **Version** 1.0.131 in package.json, tauri.conf.json, Cargo.toml, install-real.cmd. **Docs:** V14_FIXES_AND_CODE §"SCRUB IS MOVEMENT; TOASTS DO NOT STACK", WHAT-CHANGED row 1.0.131. **Gates:** cargo test --release --lib 818 passed / 0 failed / 10 ignored; cargo clippy --release --lib 0 warnings; tsc 0; vite clean; node scripts/{toast-registry 6, live-toast 8, touchpad-page 5, setting-subs 4} all pass. **NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT** — the lead does that. **UNPROVEN on hardware:** every slide and every pill (the agent cannot inject touch reports or see the overlay); the proof recipe is in the V14 section.

## 2026-09-20 — Claude (Touchpad seek + presets 1.0.130 agent, Fable) — **1.0.130: the top band IS the seek bar (`BandAction::Seek`, Windows' media session), five preset shortcut pairs in the "Does what" list, and scrub's runaway rate capped.** Owner decisions 2026-09-20 00:50 + 00:58. **SEEK.** `src-tauri/src/touchpad/seek.rs` (new): on Enter, ONE WinRT read on the reader thread (MTA) — `GlobalSystemMediaTransportControlsSessionManager::RequestAsync().get()` → `GetCurrentSession()` → `GetPlaybackInfo().Controls().IsPlaybackPositionEnabled()` → `GetTimelineProperties()` — records `P0` (Position − StartTime) and `L` (EndTime − StartTime); every Move computes `target = clamp(P0 + t × L × span, 0, L)` with `t` = travel ÷ band length (−1..1) and `span = seek_span(sensitivity)` (1 → 0.25, 2 → 0.4375, 3 → 0.625, 4 → 0.8125, **5 → 1.0**, 6 → 1.2, 7 → 1.4, 8 → 1.6, 9 → 1.8, 10 → 2.0; piecewise linear via `sens_scale`); NO acceleration. `TryChangePlaybackPositionAsync(StartTime + target)` fire-and-forget (the `IAsyncOperation` is dropped, never awaited — the raw-input pump is blocked for one call at most), gated by the pure `SeekLimiter` (≤ 10 Hz AND the target moved ≥ 1 s from the last SENT). Pill "▶ 12:34 / 45:00" (`fmt_time`: mm:ss, h:mm:ss at ≥ 1 h) updated in place; the page's readout shows the clock with the position meter. **FALLBACK:** no manager / no session / position control off / no timeline length / any call failing → `SeekSession::open` = `None` → that gesture runs as `Drive::SeekFallback` = Scrub's code path exactly, pill "⏩ Scrubbing (no seek bar here)", the page says "Scrubbing · no seek bar here". Seek is the top band's DEFAULT for a fresh config (`Band::for_edge`); Scrub stays a separate, user-selectable action — the fallback never replaces the user's choice (coordinator's (b)). Marker `touchpad-seek-bar-anchored-spaceadom-130` on Enter; Exit logs the send count and last target. **PRESETS.** `BandAction::Preset { id: PresetId }` (`{"preset":{"id":"tabs"}}` on the wire; `PresetId` = tabs · zoom · undo_redo · copy_paste · track), resolved by `touchpad/presets.rs::spec` (new, pure): Tabs Ctrl+Tab / Ctrl+Shift+Tab, Zoom Ctrl+= (0xBB) / Ctrl+− (0xBD), Undo/Redo Ctrl+Y forward / Ctrl+Z back, Copy/Paste Ctrl+V forward / Ctrl+C back, Track 0xB0 / 0xB1 — stepped ones drive exactly as Chords (`chord_steps`, one chord per notch, `send_chord`); Copy/Paste and Track are `once_per_slide` through the pure `OnceLatch` (first non-zero notch fires in its sign, then nothing until lift). Track first tries the media session's `TrySkipNextAsync`/`TrySkipPreviousAsync` (`seek::skip_track`, logs "via media-session" / "via media-key"), else presses the VK. Pill "⌨ Tabs · 3" / "⌨ Tabs · −2" (stepped), "⌨ Copy / Paste" (once). The reader's per-gesture state is now a `Drive` enum resolved once on Enter (`Drive::open`). **SCRUB CAP (owner 00:58).** `actions::scrub_rate(travel, sensitivity, band_length)`: dead zone 3 % kept, then linear from 2 taps/s to `scrub_cap(sensitivity)` (1 → 3, 2 → 4.25, 3 → 5.5, 4 → 6.75, **5 → 8**, 6 → 9.4, 7 → 10.8, 8 → 12.2, 9 → 13.6, 10 → 15 taps/s) reached at `SCRUB_RAMP_FRACTION` = 0.40 × the band's length, then FLAT; `SCRUB_MAX_RATE` (25) is gone; the speed bar is rate ÷ cap. **PAGE** (`src/components/touchpad-page.ts`): "Does what" = Brightness · Volume · Video seek ("The band is the seek bar — the video follows your finger") · Video scrub ("5-second hops, works in every player") · Tabs · Zoom · Undo / Redo · Copy / Paste · Track (each with its one-line description) · Any shortcut · Nothing, existing `.sp-row .label/.sub` styles, no new tokens; a picked preset shows its two chords read-only under "The shortcuts" with "Pick Any shortcut to change the keys — it starts from these", and picking Any shortcut over a preset seeds the recorder with that pair; `defaultBand("top")` = seek; first-run card: "Turn on the top edge and a video follows your finger."; `TouchpadLive.seek` added; `PRESETS` table mirrors the Rust one. `types.ts`: `PresetId`, `BandPreset`, `"seek"`, kinds `seek`/`preset`. Settings' slide-toast description mentions the clock. **TESTS** (all new, pure unless said): `seek::{the_span_table_is_the_documented_one, the_target_is_pure_proportion_clamped_at_both_ends_in_both_signs, the_seek_limiter_needs_a_second_of_movement_and_at_most_10_hz, time_formats_as_mm_ss_or_h_mm_ss}` + `#[ignore]` READ-ONLY `print_current_media_session_on_this_machine`; `presets::{every_preset_resolves_to_the_owners_pair, the_once_per_slide_latch_fires_on_the_first_notch_only}`; `schema::touchpad_tests::seek_and_preset_round_trip_and_older_touchpad_configs_still_load` (every id, unknown id is an error, 1.0.120/122/123 shapes load, fresh top = Seek); `touchpad::tests::the_live_toast_text_for_seek_and_presets`; `actions::{scrub_rate_is_flat_past_forty_percent_of_the_band_and_capped_per_sensitivity, scrub_rate_is_monotonic_and_sensitivity_only_raises_the_cap}` re-pinned; `scripts/touchpad-page.test.ts` gained the list-order / preset-table / copy pins. **Gates:** `cargo test --release --lib` **818 passed / 0 failed / 10 ignored**; `cargo clippy --release --lib` 0 warnings; `npx tsc --noEmit -p .` 0; `npm run build` clean (the Scrub row copy present in `dist2/assets/main-*.js`); `node scripts/{touchpad-page (5), setting-subs (4), live-toast (6)}.test.ts` all pass. **MEASURED ON THIS MACHINE, READ-ONLY (the ignored test; nothing seeked, nothing skipped):** current session app id `Chrome.UserData.Profile1`, status 5 (paused), timeline start 0:00 / position 0:00 / end 3:28 (raw 0 / 0 / 2080210000 ticks), `IsPlaybackPositionEnabled = true`, next/previous enabled — so the manager, the session and the timeline read correctly here and a seek gesture would be LIVE. **Decisions where the brief was silent:** (1) the pill keeps the ⌨ prefix for presets (a bare "Tabs" would put a "T" letter disc on the pill through `splitGlyph`); (2) `t` is travel ÷ the band's own length, clamped ±1, so the "band spans the whole video" holds for any length; (3) a timeline with zero length (live streams) falls back to scrub rather than seeking into nothing; (4) `SeekLimiter` measures the 1 s against the last position SENT (not the last computed), so a slow finger still lands within a second; (5) Track's fallback VK pair is also what "Any shortcut" is seeded with when editing it; (6) the page's `action` stays `"seek"` during a fallback gesture (with `seek: null`) so the readout label is honest; (7) the once-per-slide readout says "once per slide" where the step counter would be; (8) `Drive` is resolved on Enter, so a config change mid-slide cannot swap a gesture's action (same rule as the gesture snapshot); (9) the scrub ramp uses the band's LENGTH (the brief's "40 % of the band"), so a short band reaches the cap sooner. **UNPROVEN on hardware:** every slide — the seek following a finger, the in-place clock, the fallback pill, the presets' chords landing in an app, Track through the session, the new scrub feel. **NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT** — the lead does that. Files: `src-tauri/src/touchpad/{seek.rs (new), presets.rs (new), mod.rs, actions.rs}`, `src-tauri/src/config/schema.rs`, `src-tauri/Cargo.toml` (`Media_Control` feature + version), `src/types.ts`, `src/components/{touchpad-page,settings-panel}.ts`, `scripts/touchpad-page.test.ts`, `README.md`, `V14_FIXES_AND_CODE.md` §"TOUCHPAD — SEEK + PRESETS", `all-versions/WHAT-CHANGED.md` 1.0.130 row, version → 1.0.130 in `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `scripts/install-real.cmd`.

## 2026-09-20 — Claude (PROBLEM 272 agent, Fable) — **1.0.128: every cross-kind install pair detected, and the unpackaged side gets a one-click removal of the Store package.** Owner request 2026-09-19/20, after a friend with the Store copy installed the setup.exe on top and had two copies with no button on either side. **What changed.** (1) DETECTION — `rival_install::classify_cross_kind_all(&CrossKindInputs)` (pure, seven booleans: `we_are_packaged`, `we_are_per_machine`, `per_user_exe_exists/_is_us`, `per_machine_exe_exists/_is_us`, `store_package_registered`) with two new verdicts, `PerMachineBesidePackaged` (the Store copy sees a Program Files `.msi` install — it used to be found by Path 1 before the classifier was consulted, i.e. by accident; now the classifier names it and `detect_full` skips Path 1 while packaged) and `StoreBesidePerMachine` (the `.msi` copy sees a Store package — it used to return `None` at the top of `detect_full`; now it runs `detect_cross_kind(&me, true)` and nothing else, never Path 2, whose `classify_msi_entry` would read its own HKLM registration as an `OrphanedEntry`). The four-argument `classify_cross_kind` is a thin wrapper with the per-machine inputs false — byte-identical for all 16 of its combinations, pinned. The nine `existing × installing` pairs are tabulated in the module comment and asserted one cell at a time (`the_nine_install_pairs_each_reach_the_documented_verdict`); all 128 input combinations are checked against an independently written reference (`every_cross_kind_input_combination_matches_the_reference_rules`). EXE↔MSI is untouched: Path 1 + `plan_removal` from the per-user side, `None` from the `.msi` side. (2) REMOVAL — `rival_install::remove_store_package(full_name) -> Result<(), String>`: on a worker thread `st-store-remove`, `PackageManager::new()` → `FindPackagesByUserSecurityIdWithPackageTypes("", PackageTypes::Main)` (the WinRT `FindPackagesForUserWithPackageTypes` overload) → the full name must be in that list AND pass `is_our_store_package_name` (pure: last dot-segment of the identity Name equals `Spaceadom`, case-insensitive — `12345NurIfranArpon.Spaceadom`, `LOCALTEST.Spaceadom`, bare `Spaceadom`; NOT `Vendor.SpaceadomBeta` / `Spaceadom.Helper`; the detector used `contains("spaceadom")` and now uses this stricter rule too, so the banner names only what the button may remove) → `RemovePackageWithOptionsAsync(full, RemovalOptions::None)` → `.get()` → non-zero `ExtendedErrorCode` is a failure → verified by `find_store_package()` returning `None`, marker `store-copy-removed-by-unpackaged-side-spaceadom-128`. `Finding` gained `store_full_name` (the `path` stays a sentence). `repair()`'s `store_copy` arm calls it instead of refusing; the `is_packaged()` refusal above it is unchanged and comes first. **No package family name constant was added:** `Identity/Name` is `{{IDENTITY_NAME}}` in the committed manifest, filled from gitignored `identity.json` at pack time, and is in the binary in no form — so there is nothing to derive one from; the name rule is documented on `is_our_store_package_name`. (3) BANNER — `src/components/rival-banner.ts` (new leaf, pure `rivalBannerCopy(kind, portable, version, path)` → text / button / toasts / confirm), pinned by `scripts/rival-banner.test.ts` (10 tests, `node scripts/rival-banner.test.ts`); `main.ts::checkRivalInstall` keeps the DOM and the invokes only. `store_copy`: "A Microsoft Store copy of Spaceadom is also installed — remove it? … no permission prompt." + **Remove the Store copy** → `askConfirm` (one confirm, the app's own dialog — `window.confirm` never renders, PROBLEM 106) → `repair_rival_install` → toast "✅ Store copy removed…" or "⚠️ Could not remove the Store copy — Remove it from Settings > Apps > Installed apps, or keep it and uninstall this copy instead." `packaged_host`: directions only, now worded "remove this copy from Installed apps, or keep it and uninstall the other", button still **Open Installed apps**. Every other arm verbatim. **Files:** `src-tauri/src/rival_install.rs`, `src-tauri/src/commands.rs` (doc comments), `src/components/rival-banner.ts` (new), `scripts/rival-banner.test.ts` (new), `src/main.ts`, `CLAUDE.md` (MSIX bullet), `V14_FIXES_AND_CODE.md` §PROBLEM 272, `all-versions/WHAT-CHANGED.md` row 1.0.128, version → 1.0.128 in `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `scripts/install-real.cmd`. **Gates:** `cargo test --release --lib` 809 / 0 failed / 9 ignored (33 in `rival_install`, 7 new); `cargo clippy --release --lib` 0 warnings; `npx tsc --noEmit -p .` 0; `npm run build` clean ("Remove the Store copy" present in `dist2/assets/main-*.js`); `node scripts/rival-banner.test.ts` 10 / 0. **No-elevation claim, confidence HIGH on the documented behaviour, from:** `RemovePackageAsync` removes the package for the CURRENT USER (the windows 0.58 metadata offers the per-user `FindPackagesByUserSecurityId*` family unelevated, and the all-users `FindPackages()` is the one the crate's own Cargo.toml comment already records as needing administrator); `Remove-AppxPackage` without `-AllUsers` is the same request and runs from a non-elevated PowerShell — it is how Settings > Apps removes a Store app with no UAC. A PACKAGED caller would need the `packageManagement` restricted capability; ours refuses before it gets there. Not re-confirmed against a live Microsoft Learn URL this session (offline). **UNPROVEN:** `remove_store_package` has never executed anywhere — no Store copy on this machine, and running `RemovePackage` here was forbidden by the brief. Also UNPROVEN: the `.msi` copy's new `detect_cross_kind(&me, true)` branch on a real per-machine install; the packaged side's `PerMachineBesidePackaged` on a real machine. Not built as an installer, not installed, not committed. **QUESTIONS (one line each):** Should `store_copy` also offer "Open Installed apps" as a second door beside the removal button, or stay one-button? Should the `.msi` copy ever learn to see a per-user copy (today `None`, per the brief's "keep today's path untouched")?

## 2026-09-19 — Claude (App volume 1.0.126 agent, Fable) — **1.0.126 (same build): two new DEFAULT specials, "App volume down" / "App volume up" on Space+`-` / Space+`=` — ±10 points on the AUDIO SESSION of the app in front (its Volume-Mixer slider), never the master.** Owner request 2026-09-19. **Where it lives:** `src-tauri/src/engine/actions/app_volume.rs` (new). **The app:** `GetForegroundWindow` → pid → `exclusions::process_path_for_pid` → stem; desktop/shell (`focus::is_shell_surface`), our own pid/stem or an unreadable path → toast "No app in front". **The match:** browsers, Discord and Spotify play from HELPER processes, so every running process with the SAME EXE NAME as the foreground exe (Toolhelp snapshot, the `conflicts::detect` loop) is matched — pure `matching_pids(fg_pid, fg_exe, snapshot)` (fg pid first, case-insensitive, pid 0 never, an empty name matches by pid alone). **The sessions:** `IMMDeviceEnumerator::EnumAudioEndpoints(eRender, ACTIVE)` — EVERY active render endpoint, not only the default (owner decision 2026-09-19: with Sonar an app often plays on a non-default channel) — then per endpoint `IAudioSessionManager2` → `IAudioSessionEnumerator`; each `IAudioSessionControl2::GetProcessId` against the matched pids, expired sessions skipped; each match's `ISimpleAudioVolume` read, `next_pct(cur, ±10)` clamped 0..=100, written. Toast "🔉 Brave 40%" / "🔊 Brave 60%" with `focus::short_name_for` and the foreground pid's own session's level (else the first helper's); no live session → "🔇 <App> isn't playing anything"; any COM error → logged with the HRESULT, same toast; marker `app-volume-session-slider-moved-spaceadom-126`. `CoInitializeEx(MTA)` per press as `audio_output` does; nothing elevates; `IAudioEndpointVolume` is never touched. **Wiring:** `schema::SPECIAL_IDS` += both (17); `DEFAULT_SPECIALS` += `("minus","app_volume_down")`, `("equal","app_volume_up")`; `LATE_SINGLES` += both (pass 3, each key on its own terms); `UNSEEDED_SPECIALS` stays empty; `engine/specials.rs` rows `("app_volume_down","App volume −10%","App −","🔉")` / `("app_volume_up","App volume +10%","App +","🔊")`; `run_special` arms → `handle_app_volume(∓DELTA_PCT)` (the `handle_osk` shape); `actions/mod.rs` `pub mod app_volume`. No new crate feature — `Win32_Media_Audio` and `Win32_System_Diagnostics_ToolHelp` were already on. **Frontend:** `types.ts` `SPECIAL_IDS` (17); `special-cards.ts` two cards ("Turn down/up the app in front by 10% — its own slider in the Volume Mixer…", combo `␣ -` / `␣ =`) and `SPECIAL_SHORT` "App −" / "App +"; `preview.ts` seeded fixture now lists `\`, `-`, `=` like the app (the 1.0.125 PgDn stand-in for next_speaker removed). **ONE THING THE BRIEF SAID THAT THE MECHANISM DOES NOT:** the HUD/ring rows come in `KEY_TABLE` order (`non_letter_bindings`), and `-` (row 18) and `=` (19) sit BEFORE `\` (22), so the inner band reads `… → - = \`, not `… → \ - =`; the table is append-only (its index IS the ring's PUA code) so I followed the mechanism, not the brief. PUA codes: `-` U+E012, `=` U+E013. **Tests:** `app_volume::tests::{matching_pids_takes_the_foreground_and_every_same_named_helper, matching_pids_with_no_name_or_no_snapshot_matches_by_pid_alone, next_pct_clamps_at_both_ends, scalars_round_to_whole_percents, the_toasts_name_the_app_and_the_new_level}` + `#[ignore]` READ-ONLY `list_audio_sessions_on_this_machine`; `schema::pass_three_seeds_app_volume_on_minus_and_equal_each_on_its_own_terms` (new). Re-pinned for 17 seeds: `specials.rs` ×4 (rows `-`,`=`,`\` after `→`; `expected` sorted by `table_index` since tile order is table order), `engine::band_gate_tests::step_4_…` (15+2 rows, 21 ring, 17 preview), `hook::keys::tests::the_seed_keys_are_the_first_rows_in_seed_order` (first 14 contiguous, then `\`/`-`/`=` against rows 22/18/19) and `the_bitmap_…` (pass 3 on an empty seeded profile: 5), `middle_ring` (15 tiles / 14 after Esc / 26 codes), `schema::the_second_pass_…` (17 fresh, 18 backfilled). **Gates:** `cargo test --release --lib` **802 passed / 0 failed / 9 ignored**; `cargo clippy --release --lib` **0 warnings**; `npx tsc --noEmit -p .` **0**; `npm run build` clean. **Measured on the owner's laptop, READ-ONLY (the ignored test; nothing set), twice:** default endpoint only — 7 sessions: powertoys.peek.ui 100%, discord (32076) 100%, [system process] pid 0, msedgewebview2 100%, discord (19004) 100%, chrome (4504) 90%, textinputhost 100%, all inactive; foreground "Chrome" pid 9976 → 41 same-name pids incl. 4504. Then every active endpoint — **20 sessions across 6 endpoints**: the four Sonar Gaming/Chat/Aux/Microphone channels each hold [system process] + msedgewebview2; Speakers (Realtek) the 7 above; **Sonar Media holds gahighlight.exe, BOTH discord pids, [system process], msedgewebview2** — Discord sits on two endpoints, which is why the default-only walk was dropped. **That is the proof the helper rule is load-bearing:** by window pid alone Chrome has NO session and would toast "isn't playing anything" while its slider sits at 90%. **UNPROVEN:** the write (`SetMasterVolume` has not been issued by this code on this machine — the owner said never change a volume); the press-to-toast path in the installed app; behaviour while a session is ACTIVE (only inactive ones were present at both listings); the same app on several endpoints moving together in the Mixer. First press on the installed build: play something in Chrome, Space+`-`, expect "🔉 Chrome 80%", the marker in debug.log, and the Mixer's Chrome slider at 80. **Files:** `src-tauri/src/engine/actions/{app_volume.rs (new), mod.rs}`, `src-tauri/src/engine/{mod.rs,specials.rs}`, `src-tauri/src/config/schema.rs`, `src-tauri/src/hook/keys.rs`, `src-tauri/src/middle_ring.rs`, `src/types.ts`, `src/components/special-cards.ts`, `src/preview.ts`, `README.md`, `V14_FIXES_AND_CODE.md` §"APP VOLUME", `all-versions/WHAT-CHANGED.md` 1.0.126 row extended, this entry. Version stays 1.0.126. **NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT** — the lead does that.

## 2026-09-19 — Claude (Next speaker default-key 1.0.126 agent, Sonnet) — **1.0.126: Next speaker seeded onto Space+\ by default; nine tests updated for the fifteenth seeded special.** Owner had already added `("backslash", "next_speaker")` to `DEFAULT_SPECIALS` (now 15 entries), a third seeding pass (`LATE_SINGLES`) to backfill it into already-seeded profiles, and emptied `UNSEEDED_SPECIALS` — landing `next_speaker` on a real key one version after 1.0.125 shipped it live but unbound. **What was broken and fixed:** the owner's own new test, `pass_three_seeds_next_speaker_on_backslash_once`, panicked because `AppConfig::default()` has `profiles: Vec::new()` — indexing `cfg.profiles[0]` before giving it a profile always panics; fixed by inserting a fresh unseeded `Profile` before calling `seed_specials`, the same pattern every neighbouring seeding test already uses. **Nine pre-existing tests pinned the old counts/lists and needed their expectations moved, not the production code:** `engine::specials::tests::{a_seeded_profile_yields_todays_hud_rows, the_hud_rows_follow_the_bindings, a_seeded_profile_yields_todays_ring_tiles_with_stable_codes, step_4_the_space_ring_shows_only_apps_and_specials}` (added the "\\" / "Next speaker" row in table-index order — after "→", before the gesture rows; ring tile "Next speaker" after "Window →" at code `\u{E016}`, the key-table index of `backslash`, 22 — not contiguous with U+E00D because backslash already had its own row for typing `\`, long before this feature); `engine::band_gate_tests::step_4_a_mixed_profile_draws_only_its_apps_and_specials` (18 → 19 whole-ring rows, `pv.specials.len()` 14 → 15); `hook::keys::tests::the_bitmap_is_built_from_the_active_profile_and_special_keys` (pass-3 now also seeds backslash on an empty already-seeded profile: 2 → 3 bindings); `middle_ring::{favourites_and_payload_tests::all_of_them_appends_the_rest_and_the_specials_in_order (23 → 24), route_and_special_tests::every_ring_special_has_a_combo_and_a_private_code (12 → 13 tiles, 11 → 12 after removing Esc)}`. **One test's ASSUMPTION was wrong, not just its numbers — read as a real bug in the test, not production:** `hook::keys::tests::the_seed_keys_are_the_first_rows_in_seed_order` asserted `KEY_TABLE[i] == DEFAULT_SPECIALS[i]` for every seeded special, which held for the first 14 (contiguous append order) but cannot hold for the 15th — `backslash` is `KEY_TABLE` row 22 (`VK_OEM_5`, present since the board could type `\`), and `KEY_TABLE` is append-only/never-reordered (its index IS the ring's PUA code, `hook/keys.rs`'s own module doc). Rewrote the test to check the first 14 against their rows as before and check `backslash` against its own stable `table_index` (22) instead of row 14 — production `DEFAULT_SPECIALS`/`KEY_TABLE` were correct as the owner left them; the test's blanket assumption just never anticipated seeding a special onto a key that already existed in the table for typing. **Gates:** `cargo test --release --lib` from `src-tauri` — **796 passed, 0 failed, 8 ignored**; `cargo clippy --release --lib` — **0 warnings**; `npx tsc --noEmit -p .` from the repo root — **0 errors**. **Files:** `src-tauri/src/config/schema.rs` (test fixture fix only), `src-tauri/src/engine/specials.rs`, `src-tauri/src/engine/mod.rs`, `src-tauri/src/hook/keys.rs`, `src-tauri/src/middle_ring.rs`, `all-versions/WHAT-CHANGED.md` 1.0.126 row, this entry; version → 1.0.126 in `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml` (+ `Cargo.lock` refreshed by `cargo check`), `scripts/install-real.cmd`. **NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT** — the lead does that.

## 2026-09-19 — Claude (Next speaker 1.0.125 agent, Fable) — **1.0.125: a new special, "Next speaker" (`next_speaker`), on NO key by default — each press moves Windows' default output to the next connected speaker/headphones and toasts its name.** Owner request 2026-09-19. **Where it lives:** `src-tauri/src/engine/actions/audio_output.rs` (new). **The list:** `IMMDeviceEnumerator::EnumAudioEndpoints(eRender, DEVICE_STATE_ACTIVE)` — connected AND enabled render endpoints only, in Windows' order; id from `IMMDevice::GetId` (CoTaskMem, freed), name from the property store's `PKEY_Device_FriendlyName` (declared locally as `{a45c254e-…}, 14` — no new feature flag); the current default from `GetDefaultAudioEndpoint(eRender, eConsole)`. The PURE `next_index(current, ids)` picks the one after the current, wrapping; `Some(0)` when the current default is not in the active list; `None` for fewer than two devices → toast "🔊 Only one speaker connected" (zero devices: "🔊 No speaker connected"). **Setting the default:** Windows has no documented API for it; the module uses the Sound panel's own private COM object — `PolicyConfigClient` CLSID `{870af99c-171d-4f9e-af0d-e63df40c2bc9}`, `IPolicyConfig` IID `{f8679f50-850a-41cf-9c72-430f290290c8}`, `SetDefaultEndpoint(deviceId, role)` — the undocumented-but-standard interface SoundSwitch / nircmd / EarTrumpet have called since Windows 7. Declared with `windows-core`'s `#[interface]` macro, all twelve vtable methods in order so the slot is right, only `SetDefaultEndpoint` ever called, for all THREE roles (eConsole, eMultimedia, eCommunications) so every app follows. Any COM failure is logged with the HRESULT and toasted "🔊 Windows refused to switch" — never a panic, no elevation. Success toasts "🔊 → <friendly name>" and logs the marker `next-speaker-default-endpoint-switched-spaceadom-125`. **Wiring:** `schema::SPECIAL_IDS` += `next_speaker` and a new `UNSEEDED_SPECIALS = ["next_speaker"]` (the seed-table test now asserts `DEFAULT_SPECIALS + UNSEEDED = SPECIAL_IDS` and that an unseeded id is on exactly ZERO keys); `engine/specials.rs` row `("next_speaker", "Next speaker", "Next speaker", "🔊")` so the HUD, the icon ring and the toast name it through the existing `binding_name` path; `engine::run_special` arm → `handle_next_speaker` → `actions::audio_output::next_speaker()` → `show_toast`; `actions/mod.rs` `pub mod audio_output`; `Cargo.toml` gains `windows-core = "0.58"` as a DIRECT dependency (the `#[interface]` macro expands to `::windows_core::` paths — same 0.58.0 already in the lock, nothing new downloaded). **Frontend:** `types.ts` `SPECIAL_IDS` += `next_speaker`; `special-cards.ts` card ("Next speaker" — "Cycle to the next connected speaker or headphones. Every app follows; press again to keep going round.", `resolveSpecials` shows "Not on any key — assign it from any key's editor" until bound) and `SPECIAL_SHORT.next_speaker = "Speaker"` (the board's word under the key); the key editor's "Spaceadom special" list (Advanced mode) picks it up from `SPECIALS` unchanged; `preview.ts` binds it on PgDn (the board has no F-row) so the word and the card render. **Tests:** `audio_output::tests::{next_index_wraps_and_skips_the_current_default, next_index_with_one_or_no_device_is_none, next_index_when_the_default_is_not_in_the_active_list_starts_at_the_top, the_toasts_name_the_speaker}` + an `#[ignore]` READ-ONLY `list_active_render_endpoints_on_this_machine` (run by hand with `--ignored --nocapture`; it never sets anything); the existing `every_special_id_has_a_row_and_todays_names_are_kept` and `the_seed_table_covers_every_special_exactly_once` pass with the new id. **Files:** `src-tauri/src/engine/actions/{audio_output.rs (new), mod.rs}`, `src-tauri/src/engine/{mod.rs,specials.rs}`, `src-tauri/src/config/schema.rs`, `src-tauri/Cargo.toml`, `src/types.ts`, `src/components/special-cards.ts`, `src/preview.ts`, `README.md`, `V14_FIXES_AND_CODE.md` §"NEXT SPEAKER", `all-versions/WHAT-CHANGED.md` 1.0.125 row; version → 1.0.125 in `package.json`, `tauri.conf.json`, `Cargo.toml`, `scripts/install-real.cmd`. **Gates:** `cargo test --release --lib` 795 / 0 failed / 8 ignored; `cargo clippy --release --lib` 0 warnings; `npx tsc --noEmit -p .` 0; `npm run build` clean; `node scripts/setting-subs.test.ts` passes. **Measured on the owner's machine (read-only, from the ignored test):** 6 active render endpoints — five SteelSeries Sonar virtual devices (Gaming, Chat, Aux, Media = the current DEFAULT, Microphone) and one "Speakers (Realtek(R) Audio)"; the enumerator, the friendly names and the default lookup all work; `next_index` from Sonar Media → index 5 (Sonar Microphone). **PROVEN on this machine, once, by the owner's authorised round trip** (the `#[ignore]` test `round_trip_switch_to_the_next_speaker_and_back_on_this_machine`, run by hand): original default "SteelSeries Sonar - Media" → `SetDefaultEndpoint(next)` Ok → `GetDefaultAudioEndpoint` read back "SteelSeries Sonar - Microphone" → `SetDefaultEndpoint(original)` Ok → read back "SteelSeries Sonar - Media" again. The default was left exactly where it was found. What is still UNPROVEN: the press-to-toast path inside the installed app (the engine arm and `show_toast` — the same shape as `handle_osk`), and whether the sound audibly follows in a playing app; grep the marker above on the owner's first press. **CONDITION worth knowing:** with Sonar installed, the cycle walks five virtual outputs to reach the real speakers — see the QUESTION in the report about skipping virtual devices or a tick-list. Two things found and NOT touched: `src-tauri/src/touchpad/volume.rs` is a 22-line file cut off mid-doc-comment, not declared as a module anywhere (the working volume code is in `touchpad/actions.rs`); and 1.0.124 has no entry in this file, in WHAT-CHANGED or in V14_FIXES_AND_CODE. **NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT** — the lead does that.

## 2026-09-19 — Claude (live slide toast 1.0.123 agent, Fable) — **1.0.123: ONE toast that updates in place while a touchpad edge slide is live, plus its switch.** Owner request 2026-09-19: while a slide changes volume or brightness the overlay shows "🔊 Volume 62%" / "☀ Brightness 40%" (scrub: "⏩ Scrubbing"; a chords band: "⌨ Ctrl+Tab · 3 steps", signed, name alone at 0), appearing on Enter with the first value, updated in place at ≤ 8 Hz while Live, fading ~600 ms after Exit — and NEVER a new pill per tick. **Rust:** `lib.rs` gained `show_live_toast(app, key, text)` → global `toast-live {key, text}` and `end_live_toast(app, key)` → `toast-live-end {key}` (same global-emit rule as `show_toast`, which is untouched). `touchpad/mod.rs` gained the PURE `LiveToastLimiter` (`allow(now_ms)`: the first tick after `reset` always passes, then once per `LIVE_TOAST_MIN_MS` = 125 ms; a backwards clock is "too soon", never a burst) and the pure `live_toast_text(action, value_pct, chord, steps) -> Option<String>` (`None` for a `None` band); the reader calls show on Enter (bypassing nothing — the first tick), on Move only when the text CHANGED and the limiter allows, and on Exit flushes a throttled final value then ends — all gated on `cfg.slide_toast` where `cfg` is the `snapshot()` the callback already takes (no config-lock read on the raw-input callback). **Page (`toast.ts`):** `showLiveToast(key, text)` keeps ONE pill per key in a `_live` map — the first call builds it (same markup as a normal pill minus the drain animation, same dot→open entrance, `order:1` so it holds the bottom slot, `live: true`, no life clock), every later call with the same key swaps the `.msg`/`.ico` text nodes and restarts NOTHING (no relayout, no fit, no beep); `endLiveToast(key)` arms the ordinary `armEntry` leave/retire clock at `LIVE_LINGER_MS` = 600 (+ the entrance if it had not opened yet), so `retire` → `overlay_toasts_done` remains the one terminal hide. `relayout` gives a live pill depth 0 and leaves it out of the depth count, so the normal stack above it lays out exactly as if alone; `showToast` is byte-identical (its own eviction rule can still evict a live pill as the oldest of four — the next update simply rebuilds it). Listeners `toast-live` / `toast-live-end` registered in `initToastListener`, overlay page only. **Settings:** "Show a toast while sliding" (`tpslidetoast`) right after "Show the touchpad under the keyboard", under the same Precision gate, with a `setting-subs` line per state ("One small pill shows the level as you slide." / "Slides change the level silently."), a DESC box and a `TOGGLE_CHAR` entry; config `touchpad.slide_toast: bool`, `#[serde(default = "default_true")]`, mirrored in `types.ts` and `touchpad-page.ts::defaultTouchpad`; `persistConfig` → `save_config` → `touchpad::apply_config` → `CONFIG_GEN` is the whole path. **Tests:** `touchpad::tests::the_live_toast_limiter_passes_the_first_tick_then_at_most_8_hz` (100 Hz for a second lets through 8–9), `the_live_toast_text_per_action`, `config::schema::touchpad_tests::slide_toast_defaults_on_when_the_key_is_absent` (absent → true, explicit false survives `normalised()`), `scripts/setting-subs.test.ts` now has a TOGGLES table (on/off) plus an orphan check that every `SUB_LINES` key is rendered and wired, and NEW `scripts/live-toast.test.ts` pins the source (no DOM harness exists for `toast.ts`): the reuse path is checked before `createElement` and returns without appending, `showToast` never mentions the live map, `relayout`'s two live lines, both listeners and both emits, the limiter on Move and no `config().` on the callback. **Files:** `src-tauri/src/{lib.rs,touchpad/mod.rs,config/schema.rs}`, `src/components/{toast.ts,settings-panel.ts,setting-subs.ts,controls.ts,touchpad-page.ts}`, `src/types.ts`, `scripts/{setting-subs,live-toast}.test.ts`, `V14_FIXES_AND_CODE.md` §TOUCHPAD 1.0.123 addendum, `all-versions/WHAT-CHANGED.md` 1.0.123 row; version → 1.0.123 in `package.json`, `tauri.conf.json`, `Cargo.toml`, `scripts/install-real.cmd`. **Gates:** `cargo test --release --lib` 790 / 0 failed / 6 ignored; `cargo clippy --release --lib` 0 warnings; `npx tsc --noEmit -p .` 0; `npm run build` clean; `node scripts/{setting-subs,live-toast,touchpad-page,touchpad-tokens,sky-mode-exemptions}.test.ts` all pass. **UNPROVEN on hardware:** the pill's look and the in-place update on a real slide (the agent cannot inject touch reports; the overlay cannot be validated in a browser harness — CLAUDE.md), and what a live pill looks like beside a normal toast. The 1.0.122 fixes in the tree (brightness WMI class, 7 % → 12 % migration, demo band, pad centred on Space, idle pad dimmed) were kept as found. **NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT** — the lead does that.

## 2026-09-19 — Claude (touchpad 1.0.122 agent, Fable) — **1.0.122: every edge is a normal edge, every edge offers every action, "Any shortcut" is live, and the reader finally says what it sees.** Owner decisions 17:30: (1) NO reserved edge — the bottom edge is a band like the other three (the "Reserved · Coming later" pill, the `is-reserved` class, the "Later" rows and `Touchpad::normalised()`'s forced-off bottom are all gone; default: all four OFF, bottom's default action Nothing). (2) NO restricted action list — every edge offers Brightness · Volume · Video scrub · Any shortcut · Nothing. (3) "Any shortcut" is `BandAction::Chords { forward, backward }` (externally tagged, so a 1.0.120 config's plain `"scrub"` strings still load and the new one writes `{"chords":{"forward":[…],"backward":[…]}}`): as the finger slides, the signed travel — in units of the pad's SHORT side, a horizontal edge's travel divided by the aspect — is quantised by `touchpad::actions::chord_steps` (dead zone 3 % like scrub, then one step per `chord_step_size(sensitivity)`: 12 % at 1 … 2 % at 10, linear) and the reader brings a per-gesture `chord_sent` counter up or down to that target, sending the forward chord for each +1 and the backward chord for each −1 — so sliding back sends the other chord and undoes the slide out (CLAUDE.md's reversible rule). Each chord goes out as ONE batch through `engine::actions::chord::send_batch` (now `pub(crate)`; downs in order, ups in reverse, extended-key flags, cookie `0x7A7A7A7A`, `send_keys_checked`, PROBLEM 227); the scrub taps now go through that same `send_chord` path (which fixes their missing `KEYEVENTF_EXTENDEDKEY` on the arrows). **Scrub stays RATE-based** (it was continuous — `scrub_rate`, 2…25 taps/s — and the brief said keep it if so); Chords is STEP-based; they share the send path, not the step engine. `Band` lost `Copy` (a `Vec` inside) and chords are truncated to 8 keys on load (`Band::CHORD_MAX_KEYS`, the batch limit). (4) Page: picking "Any shortcut" shows two recorder fields, "Slide up sends…" / "Slide down sends…" on a vertical edge and "Slide right / left sends…" on a horizontal one, each with the key editor's recorder (`chord_record_start/poll/stop`, 100 ms polls, 15 s cap, the chord as of the last key-down) shown as key caps, a "Press the keys…" button that reads "Recording… press now" while armed, and a Clear button; the recorded keys land in the band's action as they arrive and stopping saves. The band's pill and the edge rows show the forward chord's name ("Ctrl+Tab") once one is recorded. The live readout for Chords shows the chord name where the percent was and a signed step counter ("3 steps", "−2 steps") instead of the meter — `touchpad-live` gained `chord` and `steps`. The recorder is a host capability (`TouchpadPageHost.recorder`, wired in `main.ts` to the three commands; `preview.ts` stubs it) so the page stays a leaf. (5) The Navy theme's `#touchpad-page` opt-in in `themes.css` is kept and now pinned by `scripts/touchpad-page.test.ts`; no other opt-in list names the page. **Coordinator's additional scope (the owner's 1.0.121 run — "reader started — pad 3751x2327, aspect 0.620", then one-finger edge slides did NOTHING and the log could not say why):** (a) the reader now logs `touchpad: band LIVE edge=… action=… at (fx,fy) width=… length=…` on Enter, `touchpad: band DEAD edge=… travel=… steps=… after N ms` on Exit, and every 5 s the proof line `touchpad: N reports/s, max contacts M, band entries K, live=… enabled=[…] last contact (fx,fy) (touchpad-proof-line-spaceadom-122)` — with it, "nothing happened" splits into "no reports reached the reader" / "reports came, no landing was inside a band" / "a band went live and the action did nothing". (b) `Band::default_width` 0.07 → **0.12** of the short side, range unchanged. (c) `raw::run_sink` is the SAME function the probe calls — `examples/touchpad-probe.rs` line 509 `raw::run_sink(&pad, on_report)` — so the message-only window, `RegisterRawInputDevices(0x0D/0x05, RIDEV_INPUTSINK, hwndTarget = that window)` on the calling thread, the `GetMessageW` pump and `parse_report` (contact count at the top collection, X/Y/ID/tip per Finger link collection) are identical by construction; the app's reader thread calls it after `CoInitializeEx(MTA)`, which does not touch raw input. **The differences are in the geometry the landing is tested against, and they are the likeliest cause:** the probe's `edge_of` band was 12 % of EACH axis with no length limit (left/right = 14 mm, top/bottom = 9 mm on this pad), while the app's `in_band` used width × aspect on the long axis (0.07 × 0.62 = 4.3 % of X = **5 mm**) and only the centred 70 %/80 % of the edge. At 0.12 the app's bands are now 9 mm on every edge; the length limit stays (the design's band). (d) The landing rule holds: `Gesture::feed` records where a contact landed on the report it FIRST appears in and evaluates the live candidate in the same call, so a contact whose first frame is inside the band arms on that frame — pinned by `a_contact_that_first_appears_inside_the_band_arms_on_that_report` (all four edges) and `the_bottom_edge_behaves_like_every_other_edge`. **Files:** `src-tauri/src/config/schema.rs`, `src-tauri/src/touchpad/{mod,gesture,actions}.rs`, `src-tauri/src/engine/actions/chord.rs`, `src/types.ts`, `src/components/touchpad-page.ts`, `src/main.ts`, `src/preview.ts`, `src/styles/touchpad.css`, `scripts/touchpad-page.test.ts` (new), `README.md`, version → 1.0.122 in `package.json`, `tauri.conf.json`, `Cargo.toml` and `scripts/install-real.cmd`. **Gates:** `cargo test --release --lib` 787 / 0 failed / 6 ignored; `cargo clippy --release --lib` 0 warnings; `npx tsc --noEmit` 0; `npm run build` clean; `node scripts/{touchpad-tokens,sky-mode-exemptions,setting-subs,touchpad-page}.test.ts` all pass. **UNPROVEN on hardware:** every slide — the 12 % band arming, the chord steps, the scrub taps through the shared path, the readout — the agent cannot inject touch reports. **NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT** — the lead does that.

## 2026-09-19 — Claude (Phase A step 4 implementing agent, Fable) — **1.0.120 (no bump): the Space ring trimmed back to apps + specials, and the key editor simple by default.** Owner: "we added too many functions, the Space HUD is messed up and it has become hard to do its main aim, launching apps." Two decisions, both in the tree beside the touchpad work (touchpad files untouched). **(1) SPACE RING TRIM, in the Rust derivation, not CSS:** the ring shows ONLY app/link pills (`action == None`) and the Spaceadom specials in their inner band — `engine/specials.rs` gained `hud_shows` / `is_app_or_link`; `hud_apps_for` + `hud_icons_for` filter on `is_app_or_link` (index-aligned), `hud_specials_for` walks specials only (non-letter keys in key-table order, then a special moved onto a letter, then the two gesture rows). A chord / uri / command / brightness / toggle binding still FIRES on its key (the hook bitmap and `run_binding` never read this) and simply isn't drawn. The mouse ring (`ring_specials_for`, `middle_ring`) is a different surface and is unchanged. Tests: `step_4_the_space_ring_shows_only_apps_and_specials` (a chord/uri/command/brightness/toggle letter → no row; the 12 + 2 seeded specials still there; a special on M is a row), `step_4_a_mixed_profile_draws_only_its_apps_and_specials` (mixed profile: 4 pills + 12 special rows + 2 gesture rows = 18, the four hidden bindings nowhere, preview payload agrees); `hud_apps_for_returns_only_mapped_keys…` and `the_hud_rows_follow_the_bindings` re-pinned (the uri on `7`/`d` is no longer a row/pill). **(2) KEY EDITOR SIMPLE BY DEFAULT** (`key-detail-panel.ts`): Advanced mode OFF → clicking a key opens the App or link picker directly with NO kind row; a key holding a non-app action shows one line over the picker, "This key runs *X* — turn on Advanced mode to change it." (`simpleNoteHtml`, `.ed-simple-note`), and the picker still replaces it through the ordinary replace-confirm. Advanced ON → the row shows App or link · Spaceadom special · Key combo · Controls · Run command. The Key-combo first-time hint logic is untouched. **(3) Settings** Advanced-mode description: "Unlocks Spaceadom specials, Key combo, Controls and Run command in the key editor." Gates: **781 unit tests / 0 failed / 6 ignored** (779 + 2), clippy 0, tsc 0, vite clean, node tests pass. §PHASE A — STEP 4 in V14_FIXES_AND_CODE.md. NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT — the lead does that.

## 2026-09-19 — Claude (Touchpad T2 implementing agent, Fable) — **1.0.120: the touchpad edge feature + its page.** ONE finger that STARTS inside an edge band (the T1b result — two fingers can't be used, Chromium/Store/Explorer scroll through Direct Manipulation no hook sees); left = brightness, right = volume, top = video scrub (continuous, rate by travel), bottom reserved. New `src-tauri/src/touchpad/{bands,gesture,actions}.rs` + engine in `mod.rs` (promotes T1's `raw.rs` + the N=1 LandingTracker path): a supervisor thread detects the pad and starts/stops a reader thread; a PURE gesture machine (Idle/Live, corner resolution, hysteresis, signed travel); volume via Core Audio, brightness via **WMI-over-COM** (shared with `engine/actions/brightness.rs`, PowerShell kept only as fallback), scrub via ←/→ taps through `send_keys_checked`. The existing `WH_MOUSE_LL` gets ONE atomic test at its top — `touchpad::bands::BAND_LIVE` — and eats WM_MOUSEMOVE/WHEEL/HWHEEL while live (pointer frozen). Config `touchpad {...}` in `schema.rs`, all `#[serde(default)]`, normalised on load/save. Frontend page `components/touchpad-page.ts` + `styles/touchpad.css` (transcribed from `design/spaceadom-touchpad_1.html`, two-finger copy/demo → one finger; look = Chocolate/Matches-the-app via `data-look`); home thumbnail inside `#keyboard-scale` (shown only when a Precision pad is present AND the new "Show the touchpad under the keyboard" toggle is on), Special-keys pill moved to bottom-right, page reachable from Settings. Gates green: **779 unit tests / 0 failed / 6 ignored** (759 + 20 new); clippy 0; `tsc` 0; `npm run build` clean; `setting-subs.test.ts`, `touchpad-tokens.test.ts`, `sky-mode-exemptions.test.ts` pass. **EVERYTHING THAT MOVES A FINGER IS UNPROVEN ON HARDWARE** — the gesture engine, the pointer freeze, and the three actions have never run on the pad from this session (agent can't inject touch); the owner must slide. NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT — the lead does that. Non-Precision touchpad detection is NOT implemented (reported as `none`); the Settings status line's "not Precision" state is therefore never reached on this build.

## 2026-09-19 — Claude (Touchpad T1 implementing agent, Fable) — **Touchpad T1 — probe built, awaiting the owner's run.** Read-only Precision Touchpad reader + `cargo run --release --example touchpad-probe`; gates green (759 unit tests / 0 failed / 6 ignored — 754 + 5 new; clippy 0; tsc 0); NOT BUILT AS AN INSTALLER, NOT INSTALLED, NO GIT, NO VERSION BUMP

**Brief:** `docs/TOUCHPAD-BRIEF-T1.md`. Two mechanisms to prove on the owner's laptop before any feature code exists: (1) read the fingers from the Precision Touchpad's HID reports over Raw Input; (2) eat `WM_MOUSEWHEEL`/`WM_MOUSEHWHEEL` in a `WH_MOUSE_LL` hook while two fingers slide inside an edge band. Nothing in the shipped app changes behaviour.

**Files:**
- `src-tauri/src/touchpad/mod.rs`, `src-tauri/src/touchpad/raw.rs` (new) — `enumerate()` (GetRawInputDeviceList/Info → vendor/product/usage, and for 0x0D/0x05 the HidP caps: finger link collections, X/Y logical + physical ranges, unit/exponent → mm, Contact Count max), `parse_report()` (HidP_GetUsageValue per Finger collection for Contact ID/X/Y, HidP_GetUsages for Tip Switch, Contact Count at the top level), `BandTracker` (two tips, same edge band, moving within 200 ms → publishes `BAND_LIVE` / `BAND_ENTERED_AT` / `BAND_LEFT_AT` atomics), `run_sink()` (message-only window + `RegisterRawInputDevices(0x0D/0x05, RIDEV_INPUTSINK)` on the calling thread — the same recipe as `hook::register_raw_keyboard_sink`, PROBLEM 268). 5 unit tests (unit exponent, mm from cm/inch, edge bands, tracker live/dead).
- `src-tauri/examples/touchpad-probe.rs` (new) — the standalone binary. Prints to stdout and appends to `%APPDATA%\Spaceadom\touchpad-probe.log` (≤ 20 lines/s: per-report line every 100 ms, summary every 5 s). Second thread installs `WH_MOUSE_LL`; the callback touches atomics only: returns `LRESULT(1)` for wheel messages while `BAND_LIVE`, counts eaten/passed, records the first eaten tick after band entry and any passed tick within 100 ms after band exit; the raw-input side reports a tick that PASSED within 100 ms before entry as the start leak. `--list` enumerates and exits without writing the log; `--band 0.15` changes the edge fraction (default 0.12, all four edges).
- `src-tauri/src/lib.rs` — `pub mod touchpad;` (no call sites in the app). `src-tauri/Cargo.toml` — windows features `Win32_UI_Input`, `Win32_Devices_HumanInterfaceDevice`.
- `src/components/settings-panel.ts` + `src/styles.css` — the unrelated owner-visible fix: toggling Advanced mode re-rendered the panel and every segmented pill (All layout, Ring layout, Theme) slid in from segment one, because `positionSegIndicator`'s `offsetLeft` read flushed style at the stylesheet default `--ind-x: 3px` before the measured value was written. `render()` now adds `.seg-settle` (transition: none on the indicator) to every pill before `wireSegIndicators` and removes it after two rAFs. Picks never pass through it, so their slide is unchanged. No other visual change.

**Enumerated from the agent shell (read-only, needs no window):** ONE Precision Touchpad — Goodix `HID\GXTP5100&Col02`, vid 0x27C6 pid 0x01E0 ver 0x10B, usage 0x0D/0x05. Input report 30 bytes, report ID 4, 6 link collections, 5 Finger collections (indices 1..5), 17 value caps, 11 button caps. Per finger: Contact ID 0x0D/0x51 (4 bits, 0..15), X 0x01/0x30 logical 0..3751, Y 0x01/0x31 logical 0..2327, physical X 0..469 / Y 0..291 in unit 0x13 (inch) exp −2 → **119.1 × 73.9 mm**. Top level: Contact Count 0x0D/0x54 logical 0..127 (descriptor max; the pad has 5 finger slots), Scan Time 0x0D/0x56 (16 bits, unit 0x1001 exp −4 = 100 µs). The pad's Col04 is a vendor collection (0xFF00/0x01); there is no separate Contact Count Max feature read here.

**Status:** the sink loop, the report parse on live reports, and mechanism 2 (eaten ticks, leaks, lag) are UNPROVEN — the container shell cannot feed the pad, and `RIDEV_INPUTSINK` from this shell would only prove the sandbox. The owner runs it (command in the T1 report).
